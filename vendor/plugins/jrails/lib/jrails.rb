  ...
  module Helpers
    module PrototypeHelper
      ...
      def remote_function(options)
        ...
        function = "$j.ajax(#{javascript_options})"
      ...
      class JavaScriptGenerator
        module GeneratorMethods
          ...
          def insert_html(position, id, *options_for_render)
            ...
            call "$j(\"##{id}\").#{insertion}", render(*options_for_render)
          end
          ...
          def replace(id, *options_for_render)
            call "$j(\"##{id}\").replaceWith", render(*options_for_render)
          end
          
          def remove(*ids)
            call "$j(\"##{ids.join(',#')}\").remove"
          end
          
          def show(*ids)
            call "$j(\"##{ids.join(',#')}\").show"
          end
          
          def hide(*ids)
            call "$j(\"##{ids.join(',#')}\").hide"
          end

          def toggle(*ids)
            call "$j(\"##{ids.join(',#')}\").toggle"
          end
    ...
    protected
      def options_for_ajax(options)
        ...
        if options[:form]
          js_options['data'] = "$j.param($j(this).serializeArray())"
        elsif options[:submit]
          js_options['data'] = "$j(\"##{options[:submit]}\").serializeArray()"
        elsif options[:with]
          js_options['data'] = options[:with].gsub('Form.serialize(this.form)','$j.param($j(this.form).serializeArray())')
        end
      ...
      def build_update(options)
        ...
        insertion = 'append' if insertion == 'bottom'
        insertion = 'prepend' if insertion == 'top'
        "$j(\"##{options[:update]}\").#{insertion}(request.responseText);"
      end
      ...
      def build_observer(klass, name, options = {})
        ...
        javascript  = "$j(\"##{name}\").delayedObserver("
    ...
    class JavaScriptElementCollectionProxy < JavaScriptCollectionProxy #:nodoc:\
      def initialize(generator, pattern)
        super(generator, "$j(#{pattern.to_json})")
      end
    end
    ...
    module ScriptaculousHelper
      ...
      def visual_effect(name, element_id = false, js_options = {})
        ...
        javascript = "$j(\"##{element_id}\").effect(\"#{name.to_s.downcase}\""
      ...
      def sortable_element_js(element_id, options = {})
        ...
        "$j(\"##{element_id}\").sortable(#{options_for_javascript(options) unless options.empty? });"
      end
      
      def draggable_element_js(element_id, options = {})
        "$j(\"##{element_id}\").draggable(#{options_for_javascript(options) unless options.empty? });"
      end
      ...
      def drop_receiving_element_js(element_id, options = {})
        ...
        "$j(\"##{element_id}\").droppable(#{options_for_javascript(options) unless options.empty? });"
      end
